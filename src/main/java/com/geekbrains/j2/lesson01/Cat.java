...
public class Cat implements Moving {
    ...
    public Cat(String name, int runReserve, int jumpReserve) {
        this.NAME = name;
    ...
    public boolean run(int distance) {
        if (distance <= runReserve) {
            ...
            countSections++;
            System.out.printf("\u001B[34mCat %s has run %d section\n\u001B[0m", NAME, countSections);
            return false;
        } else {
            printResult();
            return true;
    ...
    public boolean jump(int height) {
        if (height <= jumpReserve) {
            ...
            countWalls++;
            System.out.printf("\u001B[34mCat %s has jumped %d wall\n\u001B[0m", NAME, countWalls);
            return false;
        } else {
            printResult();
            return true;
    ...
    public void actionSelector(Object[] obstacle) {
        for (int i = 0; i < obstacle.length; i++) {
            ...
            if (i == obstacle.length - 1) {
                System.out.printf("\u001B[32m!Cat %s finished!\n\n\u001B[0m", NAME);
            }
        }
    }

    private void printResult() {
        System.out.printf("\u001B[31mCat %s gave up :(\n\u001B[0m", NAME);
